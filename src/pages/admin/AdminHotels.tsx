...
export const AdminHotels: React.FC = () => {
  ...
  const handleOpenEdit = (h: Hotel) => {
    const validId = String(h.id || (h as any).hotel_id || (h as any)._id || '').trim();
    const safeHotel: Hotel = {
      ...h,
      id: validId || (h.name ? `htl-${h.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}` : `htl-${Date.now()}`),
    };
    setEditingHotel(safeHotel);
    setName(safeHotel.name || '');
    setCityId(safeHotel.cityId || '');
    setCityName(safeHotel.cityName || '');
    setAddress(safeHotel.address || '');
    setDescription(safeHotel.description || '');
    setStarRating(safeHotel.starRating ?? 4);
    setGoogleRating(safeHotel.googleRating ?? 4.5);
    setReviewCount(safeHotel.reviewCount ?? 0);
    setBasePrice(safeHotel.basePrice ?? 3500);
    setDistanceToTemple(safeHotel.distanceToTemple || '');
    setDarshanType(safeHotel.darshanType || '');
    setIsTopRated(safeHotel.isTopRated || false);
    setAmenitiesString(Array.isArray(safeHotel.amenities) ? safeHotel.amenities.join(', ') : (safeHotel.amenities || ''));
    setImages(safeHotel.images && safeHotel.images.length > 0 ? safeHotel.images : [
      'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=800&q=80'
    ]);
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const targetHotelId = String(editingHotel?.id || (editingHotel as any)?.hotel_id || (editingHotel as any)?._id || '').trim();
      if (editingHotel && targetHotelId && targetHotelId !== 'undefined' && targetHotelId !== 'null') {
        console.log('[AdminHotels] Submitting hotel update for ID:', targetHotelId, hotelData);
        const updated = await api.updateHotel(targetHotelId, { ...hotelData, id: targetHotelId });
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {loading ? (
                  ...
                  hotels.map((hotel, index) => (
                    <tr
                      key={hotel.id || (hotel as any).hotel_id || `hotel-${index}`}
                      id={`hotel-row-${hotel.id || (hotel as any).hotel_id || index}`}
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-right space-x-2 whitespace-nowrap">
                        {(() => {
                          const resolvedId = String(hotel.id || (hotel as any).hotel_id || (hotel as any)._id || '').trim() ||
                            (hotel.name ? `htl-${hotel.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}` : `htl-${index}`);
                          const safeItem: Hotel = { ...hotel, id: resolvedId };
                          return (
                            <>
                              <button
                                id={`btn-inventory-hotel-${resolvedId}`}
                                onClick={() => setSelectedInventoryHotel(safeItem)}
                              ...
                              <button
                                id={`btn-edit-hotel-${resolvedId}`}
                                onClick={() => handleOpenEdit(safeItem)}
                              ...
                              <button
                                id={`btn-delete-hotel-${resolvedId}`}
                                onClick={() => handleDeleteHotel(resolvedId, hotel.name)}
