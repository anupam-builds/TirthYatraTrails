...
export const HomePage: React.FC = () => {
  ...
  const [checkIn, setCheckIn] = useState<string>('2026-09-15');
  const [checkOut, setCheckOut] = useState<string>('2026-09-18');
  ...
  const selectedCityName = selectedCityObj ? selectedCityObj.name : 'Select City or Area';
  ...
  const handleHeroSearch = (overrideCityId?: string, overrideCityName?: string) => {
    const cId = overrideCityId !== undefined ? overrideCityId : selectedCityId;
    const targetObj = cities.find((c) => c.id === cId);
    const cName = overrideCityName !== undefined ? overrideCityName : (targetObj?.name || (selectedCityObj?.name || ''));
  ...
  const handleSelectDropdownCity = (city: City) => {
    ...
    <div id="home-page" className="min-h-screen bg-[#fdfbf7]">
      ...
      <section className="bg-[#0f294a] text-white pt-16 pb-20 sm:pt-20 sm:pb-24 px-4 sm:px-8 relative">
        <div className="max-w-4xl mx-auto text-center relative z-10 w-full space-y-6">
          ...
          <div className="pt-2 max-w-4xl mx-auto w-full text-left relative z-30">
            <div className="bg-white text-[#0f294a] rounded-3xl sm:rounded-full p-2.5 sm:p-3 shadow-2xl border border-orange-200/50 flex flex-col sm:flex-row items-center justify-between gap-2 relative">
              ...
              <div className="relative w-full sm:w-1/3 border-b sm:border-b-0 sm:border-r border-slate-100 px-4 py-2">
                ...
                {/* Dropdown Menu */}
                {showCityDropdown && (
                  <div className="absolute top-full left-0 mt-2 z-50 bg-white shadow-2xl border border-slate-200 w-80 sm:w-96 rounded-2xl p-2.5 max-h-80 overflow-y-auto animate-in fade-in duration-150 text-[#0f294a]">
                    {/* Quick City Search Field inside dropdown */}
                    <div className="relative mb-2">
                      <Search className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-2.5" />
                      <input
                        type="text"
                        placeholder="Search Varanasi, Kedarnath, Puri..."
                        value={cityFilterText}
                        onChange={(e) => setCityFilterText(e.target.value)}
                        className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-orange-500 font-medium"
                        autoFocus
                      />
                    </div>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span>All Sacred Cities / Any Destination</span>
                      </div>
                    ...
                    {filteredDropdownCities.map((c) => (
                      <div
                        key={c.id}
                        id={`city-option-${c.id}`}
                        onClick={() => handleSelectDropdownCity(c)}
                        className={`p-2.5 rounded-xl hover:bg-orange-50 cursor-pointer flex items-center justify-between border-b border-slate-50 last:border-0 transition-colors ${
                          selectedCityId === c.id ? 'bg-orange-50 font-bold text-orange-600' : ''
                        }`}
                      >
                        <div className="flex items-start gap-2 max-w-[200px]">
                          <MapPin className="w-3.5 h-3.5 text-orange-500 mt-0.5 shrink-0" />
                          <div>
                            <p className="text-xs font-bold text-[#0f294a] leading-tight">{c.name}</p>
                            <p className="text-[10px] text-slate-500 leading-tight mt-0.5">{c.popularFor || c.state}</p>
                          </div>
                        </div>
                        <span className="text-[11px] font-semibold bg-orange-100 text-[#ea580c] px-2 py-0.5 rounded-full shrink-0">
                    ...
                    {filteredDropdownCities.length === 0 && (
                      <div className="p-3 text-center text-xs text-slate-400">
                        No sacred city found matching "{cityFilterText}"
                      </div>
                    )}
                  </div>
                )}
              ...
              <button
                ...
                className="w-full sm:w-auto px-7 py-3.5 rounded-full bg-[#ea580c] hover:bg-[#c2410c] text-white font-bold flex items-center justify-center gap-2 shadow-lg shadow-orange-600/30 transition-all text-sm shrink-0 active:scale-95 cursor-pointer"
              >
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 pt-3 text-xs text-slate-300">
              ...
              {[
                ...
                <button
                  key={dest.name}
                  onClick={() => handleHeroSearch(dest.id, dest.name)}
                  className="px-3 py-1 rounded-full bg-white/10 hover:bg-[#ea580c] hover:text-white text-slate-200 text-xs font-medium border border-white/20 transition-all active:scale-95 cursor-pointer flex items-center gap-1"
                >
