...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (error) {
        console.error('[pages/api/admin/allowlist.ts GET error]:', error);
        return res.status(500).json({ error: error.message });
      }

      return res.status(200).json({ data, success: true });
    } catch (err: any) {
      console.error('[pages/api/admin/allowlist.ts GET exception]:', err);
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (error) {
        console.error('[pages/api/admin/allowlist.ts POST error]:', error);
        return res.status(500).json({ error: error.message });
      }
