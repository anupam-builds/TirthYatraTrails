...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    try {
      ...
      if (authError) {
        console.warn('[pages/api/admin/provision-user] auth.admin.createUser notice:', authError.message);
      } else {
        authUser = authData?.user;
      }
    } catch (adminAuthErr: any) {
      console.warn('[pages/api/admin/provision-user] Admin auth creation call warning:', adminAuthErr?.message);
    }
    ...
    if (allowlistError) {
      console.error('[pages/api/admin/provision-user] admin_allowlist upsert error:', allowlistError);
    ...
    console.error('[pages/api/admin/provision-user exception]:', err);
